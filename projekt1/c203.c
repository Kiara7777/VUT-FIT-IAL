...
*/
    if (q == NULL) // test zda existuje ukazatel
    {
        queueError(QERR_INIT);
        return;
    }
    for(int i = 0; i <= QUEUE_SIZE - 1; i++) //cyklus pro inicializaci pocatecnich hodnot v poli
        q->arr[i] = '*';                     // POZOR!!!! jeden prvek musi byt nevyuzity!!!

    q->f_index = 0;    //pocatecni indexy/"ukazatele" na pocatek a konec
    q->b_index = 0;
...
*/
    return (q->f_index == q->b_index)? 1 : 0; //fronta je prazdna jestli zacatek == konec
...
*/
    if (queueEmpty(q)) //volani funkce, zda je fronta prazdna
    {
        queueError(QERR_FRONT);
        return;
    }

    *c = q->arr[q->f_index]; //vraceni znaku lezicim na indexu f_index
...
*/
    if (queueEmpty(q)) // zda je fronta prazdna
    {
        queueError(QERR_REMOVE);
        return;
    }

    q->f_index = nextIndex(q->f_index); //posunuti zacatku/ osetreni kruhovosti
...
*/
    ...
    queueFront(q, c); //vrati znak

    queueRemove(q); // odebere ze zacatku znak
...
*/
    ...
    if (queueFull(q))      //zda je fronta plna
    {
        queueError(QERR_UP);
        return;
    }

    q->arr[q->b_index] = c; //vlozit znak na konec/prvni volne misto
    q->b_index = nextIndex(q->b_index); //posunuti konce + kruhovost
